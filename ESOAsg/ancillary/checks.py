...
def check_disk_space(min_disk_space=np.float32(default.get_value('min_disk_space'))):
    ...
    true if there is enough space and rises and error otherwise.
    ...
    if (free>check_disk_space):
        enough_space = np.bool(1)
    else:
        enough_space = np.bool(0)
        msgs.error('Not enough space on disk (the limit is set to: {0:.2f} GB'.format(check_disk_space))
    return enough_space
